...
//        @Query("select m from Member m left join m.team t") // 이렇게만 하면 team 도 count 쿼리가 나가니까
    @Query(value = "select m from Member m left join m.team t",
            countQuery = "select count(m) from Member m") // 이렇게 적어주면 카운트 쿼리를 딱 정해줄 수 있음 . (count 쿼리 분리)
    ...
    @Modifying(clearAutomatically = true) // 이걸 붙혀주지 않으면 getResultList 나 getSingleResult 같은 쿼리가 나가버림. executeUpdate() 같은거라 생각하면 됨
